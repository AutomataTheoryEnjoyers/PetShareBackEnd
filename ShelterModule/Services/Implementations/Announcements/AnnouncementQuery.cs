...
public class AnnouncementQuery : IAnnouncementQuery
{
        ...
        CancellationToken token = default)
    {
        var filteredAnnouncements =
            (await _context.Announcements.Include(x => x.Author).Include(x => x.Pet).ToListAsync(token)).
            Select(a => new
            {
                Announcement = Announcement.FromEntity(a),
        ...
        if (query.MinAge is not null)
            filteredAnnouncements =
                filteredAnnouncements.Where(a => DateTime.Now.Year - a.Pet.Pet.Birthday.Year >= query.MinAge);
        if (query.MaxAge is not null)
            filteredAnnouncements =
                filteredAnnouncements.Where(a => DateTime.Now.Year - a.Pet.Pet.Birthday.Year <= query.MaxAge);
